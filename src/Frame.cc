...
Frame::Frame(const Frame &frame)
    : mpORBextractor(frame.mpORBextractor), mpSurfvocabulary(frame.mpSurfvocabulary), im(frame.im.clone()), mTimeStamp(frame.mTimeStamp),
...
Frame::Frame(cv::Mat &im_, const double &timeStamp, ORBextractor* extractor, Surf64Vocabulary* voc, cv::Mat &K, cv::Mat &distCoef)
    : mpORBextractor(extractor), mpSurfvocabulary(voc), im(im_), mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone())
{
    // Exctract ORB
